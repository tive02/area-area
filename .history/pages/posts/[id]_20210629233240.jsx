    ...
    switch (type) {
      ...
      case "unstyled":
        inlineStyleRanges.map((inlineStyle) => {
          ...
          styleBold = inlineStyle.style;
          word = new String(text.slice(offsetBold, offsetBold + lengthBold));
          return word, styleBold;
        });
        ...
        if (styleBold === "BOLD") {
          ...
          return console.log(text.replace(word, replacerBold(word)));
        } else if (styleBold === "ITALIC") {
          function replacerItalic(word) {
            return `<i className="bg-blue-500">${word}</i>`;
          }
          return console.log(text.replace(word, replacerItalic(word)));
        } else {
        }
